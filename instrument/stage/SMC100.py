...
class SMC100(SerialInstrument, Stage):
    ...
    def get_position(self, axis=None, scale = 1e-3):
        ...
        pos = [float(x) * scale for x in pos]
        return pos
    ...
    def home(self, axis):
        ...
        """
        self._send_cmd(command='OR' , axes=axis, argument=None, expect_response=False, retry=False)
        # self._send_cmd('OR')
    ...
    def set_state(self, axis=None, state=True):
        ...
        self._send_cmd(command='MM' , axes=axis, argument=1*state, expect_response=False, retry=False)
